...
public final class RedisCache {
    ...
    public static boolean setBatchData(final List<Map<String, String>> params, final Long seconds) {
        boolean result = redisTemplate.execute((RedisConnection connection) -> {
            ...
            params.forEach(data -> data.entrySet().forEach(m -> connection.setEx(m.getKey().getBytes(), finalExpire, m.getValue().getBytes())));
            return true;
        }, false, true);
        return result;
    }
    ...
    public static List<Map<String, String>> getBatchData(final List<String> keys) {
        List<Map<String, String>> results = redisTemplate.execute((RedisConnection connection) -> {
    ...
    public static Long remove(final String key) {
        return redisTemplate.execute((RedisConnection connection) -> connection.del(serializer.serialize(key)));
    }
